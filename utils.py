...
def generate_batch_idx(idx_train, y_train, batch_size, seed):
    ...
    num_batches = int(
        labels.sum() * 2 / batch_size) + 1  # + 1 添加一个额外的批次，确保即使在样本数不能整除批次大小时，也能涵盖所有样本。这样可以避免最后一个批次的样本数量不足。
    for batch in range(num_batches):
        pos_start = int(batch * batch_size / 2)  # / 2：由于每个批次包含一半的正样本和一半的负样本，因此正样本的数量为 batch_size 的一半。
...
def test(model, features, idx, not_train_labels, adj, adj_knn, result, epoch_best, flag=None,
         valid_thresh=None):
    labels = []
    output_list = [[], [], []]
    ...
    log_probs, embeddings = model.myforward(features, adj, adj_knn)
    probs = torch.exp(log_probs[idx]).data.cpu().numpy()
    loss_test = F.nll_loss(log_probs[idx], not_train_labels.cuda())
    output_labels = not_train_labels.cpu().numpy()

    prediction = probs.argmax(axis=1)
    confidence = probs.max(axis=1)
    anomaly_confidence = probs[:, 1]
    ...
    if flag == "test":
        result.write_test_log(epoch_best, accuracy, f1, f1_macro, precision, precision_macro, recall, recall_macro, auc,
                              line)
    elif flag == "val":
...
def collect_settings(exp_result_path: str) -> list:
    return list(
        set(["-".join(filename.split('-')[:3]) for filename in os.listdir(os.path.join(exp_result_path, 'test_log'))]))
...
def load_df_test(exp_result_path: str, setting: str) -> pd.DataFrame:
    ...
    log_test_path_l = [os.path.join(log_test_dir, filename) for filename in os.listdir(log_test_dir) if
                       setting in filename]
...
def load_df_val(exp_result_path: str, setting: str) -> pd.DataFrame:
    ...
    log_val_path_l = [os.path.join(log_val_dir, filename) for filename in os.listdir(log_val_dir) if
                      setting in filename]
